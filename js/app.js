...
if (buttons.length) {
   ...
   buttons.forEach(button => {
      button.addEventListener('click', () => {
         ...
         const image = button.querySelector('.image-item')
         const label = button.querySelector('label')
         toggle(checkbox, image, label)
         let current = -1
         arrayIng.forEach((ar, index) => {
            if (checkCompare(activeButtons, ar)) current = index
         })
         const coffee = document.querySelector('.block__coffee')
         const images = coffee.querySelector('img')
         const title = coffee.querySelector('.content-coffee__title')
         const text = coffee.querySelector('.content-coffee__text')
         if (images && images.parentElement) images.parentElement.classList.remove('hide')
         if (title) title.classList.remove('hide')
         if (text) text.classList.remove('hide')
         if (coffee && current !== -1) {
            if (images) images.src = array[current].image
            if (title) title.innerHTML = array[current].title
            if (text) text.innerHTML = array[current].description
         } else {
            if (title) title.innerHTML = 'Нет подходящего кофе'
            if (images && images.parentElement) images.parentElement.classList.add('hide')
